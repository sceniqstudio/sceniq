    ...
    <div style={{ minHeight: '100vh', background: '#0a0a14', color: '#f5f2ec', fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      ...
      <main style={{ maxWidth: '720px', margin: '0 auto', padding: '56px 24px 80px' }}>
        ...
        <div style={{ background: '#7C5CFC', borderRadius: '6px', padding: '4px 12px', display: 'inline-block', marginBottom: '20px' }}>
          <span style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase' as const, color: '#fff' }}>RGPD</span>
        </div>
        ...
        <Section title="Qui sommes-nous ?">
          <p style={pStyle}>ScenIQ est une agence IA de production vidéo publicitaire, exploitée en micro-entreprise par Pascal Ekloui. Responsable du traitement des données : Pascal Ekloui — <a href="mailto:support@sceniq.studio" style={{ color: '#A5B4FC' }}>support@sceniq.studio</a>.</p>
        </Section>
        ...
        <Section title="Données collectées">
          ...
          <p style={pStyle}>Le site ne recourt à aucun cookie publicitaire ni traceur de navigation.</p>
        </Section>
        ...
        <Section title="Vos droits (RGPD)">
          <p style={pStyle}>Conformément au Règlement européen 2016/679 (RGPD), vous disposez des droits suivants :</p>
          <ul style={{ margin: '0 0 16px', paddingLeft: '20px' }}>
            {["Droit d'accès à vos données","Droit de rectification","Droit à l'effacement","Droit à la limitation du traitement","Droit à la portabilité","Droit d'opposition"].map(r => (
              <li key={r} style={{ fontSize: '15px', color: 'rgba(255,255,255,.75)', marginBottom: '6px', lineHeight: 1.6 }}>{r}</li>
            ))}
          </ul>
          <p style={pStyle}>Pour exercer ces droits, contactez-nous à <a href="mailto:support@sceniq.studio" style={{ color: '#A5B4FC' }}>support@sceniq.studio</a>. Réponse sous 30 jours.</p>
          <p style={pStyle}>Vous pouvez également adresser une réclamation à la <a href="https://www.cnil.fr" target="_blank" rel="noopener noreferrer" style={{ color: '#A5B4FC' }}>CNIL</a>.</p>
        </Section>

        <Section title="Cookies">
          <p style={pStyle}>ScenIQ n'utilise aucun cookie publicitaire ou de suivi. Le seul cookie déposé est celui de session Stripe, strictement nécessaire au traitement de votre paiement.</p>
        </Section>

        <Section title="Sécurité">
          <p style={pStyle}>Les communications sont chiffrées via HTTPS/TLS. Les données de paiement sont tokenisées par Stripe et ne transitent jamais par nos serveurs. Les fichiers uploadés sont stockés dans un bucket privé Supabase avec accès restreint.</p>
        </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section style={{ marginBottom: '40px' }}>
      <h2 style={{ fontSize: '15px', fontWeight: 700, color: '#fff', margin: '0 0 16px', paddingBottom: '10px', borderBottom: '1px solid rgba(255,255,255,.1)' }}>
        {title}
      </h2>
      {children}
    </section>
